...
def usage_from_openai_obj(usage: Any) -> Usage:
    """Parse an OpenAI-shaped usage object (Chat or Responses API).

    Shared with adapters for OpenAI-compatible surfaces (LiteLLM).
    """
...
class OpenAIAdapter(ProviderAdapter):
    ...
    #: base_url host (substring) -> provider name, for OpenAI-compatible APIs.
    _HOST_PROVIDERS = (
        ("api.x.ai", "xai"),
    ...
    def provider_for(self, client: Any) -> str:
        ...
        # base_url. Stamp the real provider so attribution isn't all "openai".
        base_url = str(getattr(client, "base_url", "") or "")
        for host, provider in self._HOST_PROVIDERS:
            if host in base_url:
                return provider
        return self.name
